...
\echo '🔧 Creando funciones RPC para pedidos...'
...
\echo '🔧 Creando funciones RPC para notificaciones...'
...
-- ========== PASO 5: RLS POLICIES ==========
\echo '🔒 Configurando RLS Policies...'

ALTER TABLE productos ENABLE ROW LEVEL SECURITY;
ALTER TABLE pedidos ENABLE ROW LEVEL SECURITY;
ALTER TABLE notificaciones ENABLE ROW LEVEL SECURITY;

-- Productos: todos pueden ver disponibles, staff puede todo
CREATE POLICY "Ver productos disponibles" ON productos FOR SELECT USING (disponible = true);
CREATE POLICY "Staff gestiona productos" ON productos FOR ALL TO authenticated USING (true);

-- Pedidos: staff ve todo, clientes solo sus pedidos
CREATE POLICY "Staff ve todos los pedidos" ON pedidos FOR SELECT TO authenticated USING (true);
CREATE POLICY "Crear pedidos" ON pedidos FOR INSERT USING (true);
CREATE POLICY "Staff gestiona pedidos" ON pedidos FOR ALL TO authenticated USING (true);

-- Notificaciones: staff ve todo, clientes pueden crear
CREATE POLICY "Staff ve notificaciones" ON notificaciones FOR SELECT TO authenticated USING (true);
CREATE POLICY "Crear notificaciones" ON notificaciones FOR INSERT USING (true);
CREATE POLICY "Staff gestiona notificaciones" ON notificaciones FOR ALL TO authenticated USING (true);


-- ========== PASO 6: DATOS SEED DE PRODUCTOS ==========
\echo '🌱 Insertando productos seed...'
...
SELECT 'Productos insertados' as tabla, COUNT(*) as total FROM productos
UNION ALL
...
\echo 'Tablas: productos, pedidos, notificaciones'
\echo 'Funciones RPC: crear_pedido, obtener_pedidos_visita, crear_notificacion, obtener_notificaciones_pendientes'
\echo 'RLS Policies: Configuradas con seguridad granular'
\echo 'Productos seed: ' || (SELECT COUNT(*) FROM productos)::TEXT || ' productos insertados';
