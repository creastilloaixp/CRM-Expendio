...
const Header: React.FC<HeaderProps> = ({ isAuthenticated, onLogout, onNavigate, currentView }) => {
  const NavButton: React.FC<{ view: NavigationView; label: string }> = ({ view, label }) => {
      ...
      <div className="container mx-auto px-4 sm:px-6 md:px-8 flex justify-between items-center h-20">
        ...
        {isAuthenticated && (
          <div className="flex items-center space-x-2">
            <nav className="hidden md:flex items-center space-x-2 bg-gray-200/50 p-1 rounded-lg">
              <NavButton view="dashboard" label="Dashboard" />
              <NavButton view="kitchen" label="🍳 Cocina" />
              <NavButton view="clientes" label="Clientes" />
       ...
       {isAuthenticated && (
          <nav className="md:hidden flex flex-wrap items-center justify-center space-x-2 bg-gray-200/50 p-2">
            <NavButton view="dashboard" label="Dashboard" />
            <NavButton view="kitchen" label="🍳 Cocina" />
            <NavButton view="clientes" label="Clientes" />
            <NavButton view="reports" label="Reportes" />
